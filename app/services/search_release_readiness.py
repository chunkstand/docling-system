...
from app.services import search_release_readiness_responses as readiness_responses
from app.services.search_release_shared import (
    latest_audit_bundle_validation_receipt as _latest_validation_receipt,
)
from app.services.search_release_shared import (
    latest_release_readiness_assessment as _latest_readiness_assessment,
)
from app.services.search_release_shared import release_package_sha256 as _release_package_sha256
from app.services.search_release_shared import (
    search_harness_release_not_found_error as _search_harness_release_not_found,
)
...
def search_harness_release_readiness_assessment_integrity(
    ...
    return readiness_responses.search_harness_release_readiness_assessment_integrity(row)
...
def _validation_error_codes(errors: list[dict]) -> list[str]:
    return sorted({str(error["code"]) for error in errors if error.get("code") not in {None, ""}})


def _failed_audit_check_keys(audit_checklist: dict) -> list[str]:
    return sorted(
        key for key, value in audit_checklist.items() if key != "complete" and value is False
    )
...
def _false_check_reasons(checks: dict) -> list[str]:
    return sorted(key for key, value in checks.items() if key != "complete" and value is False)
        ...
        details.append(
            {
                ...
                "audit_checklist_failed": list(diagnostics.get("audit_checklist_failed") or []),
                "lineage_remediation_required": bool(lineage_remediation.get("action_required")),
                "suggested_operator_action": (
    ...
    *,
    ...
    return SearchHarnessReleaseReadinessResponse(
        ...
        latest_readiness_assessment=(
            readiness_responses.to_readiness_assessment_summary(latest_assessment)
...
def create_search_harness_release_readiness_assessment(
    ...
    session.add(row)
    session.flush()
    return readiness_responses.to_readiness_assessment_response(row)
...
def get_latest_search_harness_release_readiness_assessment(
    ...
    return readiness_responses.to_readiness_assessment_response(assessment)
...
def get_search_harness_release_readiness_assessment(
    ...
    if assessment is None or assessment.search_harness_release_id != release_id:
        raise api_error(
            status.HTTP_404_NOT_FOUND,
            "search_harness_release_readiness_assessment_not_found",
            "Search harness release readiness assessment not found.",
