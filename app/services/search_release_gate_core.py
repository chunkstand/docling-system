...
def _replay_run_rank_metrics(row: SearchReplayRun) -> dict:
    return (row.summary_json or {}).get("rank_metrics") or {}


def _load_completed_replay_run(
    session: Session,
    replay_run_id: UUID,
    *,
    label: str,
...
def evaluate_search_harness_release_gate(
    ...
    for source in evaluation.sources:
        baseline_run = _load_completed_replay_run(
            session,
            source.baseline_replay_run_id,
            label=f"{source.source_type} baseline",
        )
        candidate_run = _load_completed_replay_run(
            session,
        ...
        baseline_rank_metrics = _replay_run_rank_metrics(baseline_run)
        candidate_rank_metrics = _replay_run_rank_metrics(candidate_run)
