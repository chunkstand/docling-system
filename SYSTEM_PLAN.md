...
- Evaluation-data readiness now passes both the regression and court-grade
  tiers on the rebuilt local DB. The live state includes active documents,
  persisted evaluations, auto-generated fixtures, a reviewed five-document
  manual corpus, replay coverage for every required source, harness-evaluation
...
- `draft_claim_support_calibration_policy -> verify_claim_support_calibration_policy -> apply_claim_support_calibration_policy`, which governs claim-support replay policy changes through a draft row, default/explicit fixtures plus the latest promoted replay-alert fixtures and mined prior failures, fixture replay verifier, a default verifier failure when stale replay escalation receipts remain unpromoted into fixture coverage, required operator/reason/severity/timezone-aware-expiry waiver artifacts with a 72-hour maximum lifecycle when that coverage gate is disabled, unchanged-draft hash checks, approval-gated activation, waiver hash recomputation before activation, expired/overlong-waiver activation blocking, separate waiver-specific activation approval by an operator different from both the task approver and waiver creator, prior-policy retirement, a `claim_support_policy_activation_governance` artifact with policy diff, fixture replay evidence, fixture-set diff, replay-alert fixture summary, replay-alert coverage waiver, mined-failure manifest, approval record, waiver activation approval, signed hash-chain receipt when configured, PROV JSON-LD, a linked `claim_support_policy_activated` semantic-governance event, a persisted `claim_support_policy_change_impacts` row listing prior technical-report support judgments, generated draft tasks, verifier tasks, affected IDs, replay recommendations, a reserved row ID, a verified payload hash, atomically queued/idempotent managed replay task IDs/plan/status, replay-plan and terminal closure hash checks, worker-side replay status refresh, immediate no-action terminal closures, immutable closure receipt artifacts, `claim_support_policy_impact_replay_closed` semantic-governance events, summary/stale-open impact counts, an operator worklist endpoint and Agent Workflows UI for stale/blocked replay operations, replay-alert escalation receipts, replay-alert-derived fixture candidate mining with content-stable candidate IDs, governed fixture-set promotion receipts with source/candidate coverage counts, replay-alert coverage summaries, technical-report evidence-manifest/trace visibility for fixture-promotion events, replay-aware decision signals for open/stale/blocked impacts, stale unconverted escalation receipts, and active/expiring/expired/high-severity replay-alert fixture coverage waiver posture, and a database-enforced single active policy per policy name
