...
DOCLING_SYSTEM_RUN_INTEGRATION=1 uv run pytest -q tests/integration/test_claim_support_judge_evaluation_roundtrip.py::test_claim_support_change_impact_replay_prevalidates_before_creating_tasks tests/integration/test_claim_support_judge_evaluation_roundtrip.py::test_claim_support_policy_activation_carries_replay_alert_coverage_waiver
...
claim-support action schema/API/CLI checks: 5 passed in 6.37s
claim-support replay-alert coverage/waiver integration: 2 passed in 3.86s
...
The next useful pass is waiver lifecycle control for replay-alert fixture coverage:

1. add waiver expiry/severity fields and decision-signal thresholds for aging waivers
2. require a second approval note before activating a policy whose verification used an active waiver
3. auto-suggest fixture promotion once waived escalations become promotable

That would move waivers from auditable exceptions into actively managed risk.
