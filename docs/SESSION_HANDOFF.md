...
claim-support action schema/API/CLI checks: 4 passed in 7.22s
claim-support replay-alert coverage integration: 1 passed in 2.96s
...
The next useful pass is to add an explicit waiver/audit path for any verification that disables replay-alert fixture coverage:

1. require a waiver reason, operator identity, and durable artifact when `require_replay_alert_fixture_coverage=false`
2. surface waiver state in activation governance, decision signals, and evidence manifests
3. add tests proving waived verification cannot become an invisible policy-promotion path

That keeps the new hard gate practical for emergency operations while preserving court-grade traceability.
