...
- `docs/evidence_residual_owner_family_milestone_plan.md`: active standalone follow-on for the selected residual evidence owners left open after the provenance-export split. Milestone 0 is resolved locally through closeout commit `44bec70`, Milestone 1 is resolved locally through closeout commit `d9d79ef`, Milestone 2 is resolved locally through closeout commit `115be15`, Milestone 3 is resolved locally through closeout commit `245dc9f`, and Milestone 4 is resolved locally in the current checkout: `app/services/evidence_claim_feedback.py` is reduced to `47` lines, row payload shaping plus row and integrity summary reporting now live in `app/services/evidence_claim_feedback_integrity.py` at `305` lines, row lookup plus append-only live-link enforcement and ledger persistence now live in `app/services/evidence_claim_feedback_lifecycle.py` at `215` lines, the focused claim-feedback and adjacent-caller unit slice passed at `37 passed`, the focused audit and retrieval-learning integration slice passed at `6 passed`, and the full DB-backed integration gate passed at `2009 passed`; the broader `IC-65AF4A6D8B1E` case remains intentionally open because `app/services/evidence_semantic_trace.py`, `app/services/evidence_audit_views.py`, `app/services/evidence_manifest_traces.py`, `app/services/evidence_manifests.py`, and `app/services/evidence_claim_support_replay_alerts.py` still measure `837`, `699`, `980`, `725`, and `646`. Milestone 5, `Semantic Trace Integrity And Provenance Edge Boundary`, is the next implementation slice because the semantic-trace root still cohabits technical-report derivation integrity recomputation, semantic trace materialization, source-record shaping, evidence-card source record shaping, and provenance-edge expansion.
