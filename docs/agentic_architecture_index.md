...
- `docs/cli_command_dispatch_boundary_milestone_plan.md`: Milestone 0 is resolved locally through closeout commit `381ca15` after the claim-support, evaluations, evidence provenance-export, and semantics packets closed as commits `3d7d090`, `1159297`, `1aa8378`, and `a2eb27e`. The live rebaseline still routes `IC-9812A0B138D9` / `app/cli.py` as the top hotspot at `1231` lines / `score 67705`, and Milestone 1 is now the next active CLI facade-prevention slice. This remains the current active bounded implementation brief.
