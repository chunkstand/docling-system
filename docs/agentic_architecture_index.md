...
- `docs/documents_api_route_surface_boundary_milestone_plan.md`: resolved locally in the current checkout through the broader-reselect follow-on for `IC-23F2C79C8AA7`. `tests/unit/test_documents_api.py` now closes at `155` lines, access coverage now lives in `tests/unit/test_documents_api_access.py` at `253`, run coverage now lives in `tests/unit/test_documents_api_runs.py` at `137`, latest-evaluation coverage now lives in `tests/unit/test_documents_api_evaluations.py` at `93`, the earlier artifact and semantic siblings remain at `286` and `394`, hotspot prevention now routes the reduced root as a deferred reduced facade, the full document-route owner family is exact-ratcheted, and the live queue remains empty after the broader reselect.
- `docs/remaining_packet_queue_resolution_milestone_plan.md`: active queue-coordination brief after the documents API route-surface closeout. Packet A is now resolved, Packet B verified-to-deployed closeout is next, and only Packet C `IC-3B4C9F2A76E1` plus Packet D `IC-25C1F7B9E4DA` remain as code-owning residual packets before Packet E final queue exhaustion or rebaseline.
- `docs/stale_open_registry_closeout_milestone_plan.md`: resolved through the 2026-05-19 docs-only Packet A closeout. The stale `open` cases `IC-65AF4A6D8B1E`, `IC-FD18EE2D3309`, `IC-81C531769EB3`, `IC-9A0332D41F79`, `IC-33B4990DC366`, `IC-649D7B4E3AB5`, `IC-4B6E9F8D2A10`, `IC-81F2C6D4B9A7`, and `IC-2D5A7E9C4B18` are now deployed, the live registry counts now read `open=2`, `verified=9`, and `deployed=49`, and Packet B verified-to-deployed closeout is the next queued pass.
...
- `docs/hotspot_prevention_family_boundary_milestone_plan.md`: resolved locally through closeout commit `463d3fc`. `app/hotspot_prevention_classifier.py` remains `360` lines / `1` private helper, the companion analyzer root now closes at `343 / 0`, policy and report contract coverage now lives in `tests/unit/test_hotspot_prevention_policy_contracts.py` at `317 / 0`, blocked-family coverage remains at `318 / 0`, wrapper coverage remains at `296 / 0`, and shared support remains at `50 / 2`. `IC-6C1B516A3F92` and `IC-15F6E41A9C77` are now recorded as deployed locally, `config/hotspot_prevention.yaml` routes the residual root test as a deferred reduced facade, the focused family gate passed at `41 passed`, and the broader queue now returns to `IC-03D7EFA03213` / `tests/unit/test_search_api.py`.
...
- `docs/oversized_test_hotspots_boundary_milestone_plan.md`: resolved locally through closeout commit `65c0c67` in the 2026-05-14 oversized-test closeout window, with the later 2026-05-18 technical-report harness residual closeout now fully verifying `IC-D49E037D5657` and the later 2026-05-18 agent-tasks API lifecycle closeout now deploying `IC-D9A84C20546B` through implementation commit `790fa2d` plus durable docs-and-registry closeout commit `8ce05b7`. The seven selected residual files remain `159`, `328`, `92`, `389`, `117`, `428`, and `93` lines respectively for `tests/db_model_contract.py`, `tests/unit/test_agent_task_context.py`, `tests/unit/test_agent_tasks_api.py`, `tests/unit/test_evaluation_service.py`, `tests/unit/test_search_service.py`, `tests/integration/test_retrieval_learning_ledger.py`, and `tests/integration/test_technical_report_harness_roundtrip.py`; the agent-tasks API family now closes at `360`, `360`, `566`, `419`, and `93` across the analytics, lifecycle, artifacts, claim-support, and auth siblings, the harness family now closes at `315`, `398`, `313`, `162`, `402`, and `206` across the focused context-pack, audit, audit-persistence, manifest, integrity, and source-evidence files, the architecture probe no longer lists any of those residual files in the top 20 hotspots, and only `IC-3B4C9F2A76E1` plus `IC-25C1F7B9E4DA` remain reduced/open because focused successor files still exceed the default `600`-line hygiene budget.
...
- The later 2026-05-19 registry-alignment sweep deploys the stale reduced-root
  ...
  `IC-23F2C79C8AA7` to verified at the reduced `155`-line state, bringing the
...
- The later queue-freeze follow-on records the first explicit remaining-packet
  ...
  `IC-3B4C9F2A76E1` plus Packet D `IC-25C1F7B9E4DA` remain as genuinely
  code-owning residual packets unless a later Milestone 0 rebaseline finds new
  regrowth.
- The later 2026-05-19 stale-open registry closeout completes Packet A and
  ...
  verified-to-deployed closeout is now the next queue pass and only
  `IC-3B4C9F2A76E1` plus `IC-25C1F7B9E4DA` remain open.
