...
- `docs/ci_release_gate_parity_milestone_plan.md`: refreshed locally through Milestone 0 for `IC-2D8D5BF5A8C4` and now active through Milestone 1 local runner-contract closeout in the current checkout. The runtime-health dependency is satisfied against the repo-owned `api` / `worker` / `agent-worker` health contract surfaces plus successful Compose smoke, `app/release_gate_cli.py` now owns the canonical `docling-system-release-gate-parity` command, focused runner coverage in `tests/unit/test_release_gate_cli.py` proves the step-list and compose-lifecycle contract, the runner now passes end to end locally, and the remaining scoped gap is the missing checked-in `.github/workflows/release-gate-parity.yml` workflow.
