...
- The later 2026-05-19 classifier-owner rebaseline closeout keeps the live
  improvement-case summary at `status_counts={"measured":1,"deployed":62}`,
  ...
  returns the current live architecture-quality summary to
  `stale_facade_hotspot_count=20` with `top_routed_hotspot_paths=[]` by routing
  `app/hotspot_prevention_classifier.py` as a deferred reduced facade instead
  of leaving it as the active routed owner.
