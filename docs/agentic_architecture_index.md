...
- `docs/cli_command_dispatch_boundary_milestone_plan.md`: Milestone 2 is resolved locally through closeout commit `f5a4260` after the Milestone 1 closeout `c674871` and the Milestone 0 rebaseline closeout `381ca15`; runtime and maintenance command ownership now lives in `app/cli_commands/runtime.py` at `463` lines, `app/cli.py` is reduced to `926` lines and the current probe now measures `56` revisions / `score 51856`, the broader CLI owner case remains reduced because `app/cli.py` still appears in the architecture-quality hotspot routing, and Milestone 3 is now the next active CLI retrieval-learning and search-harness command-owner extraction slice. This remains the current active bounded implementation brief.
