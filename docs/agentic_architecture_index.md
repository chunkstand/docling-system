...
- `docs/hygiene_owner_case_routing_boundary_milestone_plan.md`: active bounded follow-on after the claim-support, evaluations, evidence provenance-export, semantics, CLI, agent-task schema, and oversized-test packets. Milestone 0 is now resolved locally through baseline commit `08a1a75`; the remaining `owner_milestone=residual-weakness-milestone-2` residual set is still exactly `app/architecture_inspection.py`, `app/architecture_inspection_rules.py`, `app/services/claim_support_evaluations.py`, `app/services/claim_support_policy_governance.py`, `app/services/claim_support_replay_alert_fixture_corpus.py`, `app/services/improvement_case_intake.py`, `app/services/improvement_cases.py`, and `app/services/semantic_governance.py`; none of those files yet have explicit owner cases in `config/improvement_cases.yaml`; and Milestone 1 owner-case bootstrap is now the next active code-changing slice before the packet removes milestone-owned routing from the active hygiene contract.
