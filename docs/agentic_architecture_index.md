...
- `docs/claim_support_residual_owner_family_milestone_plan.md`: drafted standalone follow-on for the remaining claim-support residual architecture across `IC-E2270F89B397` and `IC-7C73737C689F`; Milestone 0 refreshes the live baseline, then the plan retires the policy-impact residual owners, removes the live claim-support cycle between `claim_support_policy_impacts` and `claim_support_replay_alert_promotions`, and decomposes the evaluation, governance, and replay-alert fixture-corpus support family until every governed owner closes at or below the default 600-line budget.
