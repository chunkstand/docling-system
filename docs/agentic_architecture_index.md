...
- `docs/runtime_health_orchestration_milestone_plan.md`: active stacked follow-on after the claim-support, evaluations, evidence provenance-export, semantics, CLI, agent-task schema, oversized-test, hygiene owner-case routing, and architecture-governance cycle packets. Milestone 0 refresh / owner-case bootstrap is now committed locally as checkpoint `289f15a`: `IC-0F89DBB1CF9F` anchors the production-orchestration health gap across `app/api/routers/system.py`, `app/services/runtime.py`, `app/workers/poller.py`, `app/workers/agent_poller.py`, and `docker-compose.yml`, the refreshed baseline still measures those surfaces at `85 / 194 / 15 / 15 / 307 / 110` lines, and no prior runtime-health owner case existed in the registry before this bootstrap. The shared runtime-health contract, process-heartbeat freshness, and Compose healthchecks remain unimplemented, so Milestone 1 gate-first health contract is now the next active slice.
