...
- `docs/improvement_case_governance_self_hosting_milestone_plan.md`: resolved through the 2026-05-18 durable closeout. The governed roots now close at `370`, `514`, `552`, `82`, and `218` lines, family-local siblings now close at `122`, `184`, `475`, `279`, `277`, `101`, `122`, and `551` with exact hygiene ratchets, the local `app.services.improvement_case_observations` / `app.services.improvement_cases` cycle remains removed, and the next code-owning packet must be reselected from `docs/boring_change_architecture_milestone_plan.md`.
...
- `docs/semantics_service_boundary_milestone_plan.md`: resolved locally through closeout commit `a2eb27e` on 2026-05-13 local / 2026-05-14 UTC for `IC-9E6B8F5D62A1`; `app/services/semantics.py` is now a 54-line / 0-private-helper compatibility facade, the later lifecycle/read follow-on has reduced `app/services/semantic_pass_lifecycle.py` to `529` lines / `3` private helpers, moved artifact ownership into `app/services/semantic_pass_artifacts.py` at `150` lines / `0` private helpers, kept review or projection ownership in `app/services/semantic_pass_reviews.py` at `369` lines / `4` private helpers, reduced read ownership in `app/services/semantic_pass_reads.py` to `372` lines / `3` private helpers, moved read source-materialization and record-shaping ownership into `app/services/semantic_pass_source_records.py` at `415` lines / `4` private helpers, kept registry preview ownership in `app/services/semantic_registry_preview.py` at `558` lines / `5` private helpers, and now leaves the broader owner case locally retirement-ready pending commit because no governed semantic owner remains above the default 600-line budget.
- `docs/semantic_pass_lifecycle_reads_boundary_milestone_plan.md`: Milestones 0 through 5 are resolved locally in the current checkout as the latest resolved bounded semantic-owner follow-on after the replay-alert evidence packet. The refreshed packet now keeps `app/services/semantic_pass_lifecycle.py` at `529` lines / `3` private helpers, adds `app/services/semantic_pass_artifacts.py` at `150` lines / `0` private helpers, keeps `app/services/semantic_pass_reviews.py` at `369` lines / `4` private helpers, reduces `app/services/semantic_pass_reads.py` to `372` lines / `3` private helpers, adds `app/services/semantic_pass_source_records.py` at `415` lines / `4` private helpers, keeps the cycle baseline at `3` with `29` code files above `800`, and closes the full DB-backed gate at `2037 passed` while leaving the selected owner cases retirement-ready pending commit.
...
- `docs/app_large_owner_modules_resolution_milestone_plan.md`: resolved locally in the working tree on 2026-05-15. The selected app-side large owners are now reduced to `185`, `199`, `15`, `120`, `91`, `39`, and `404` lines respectively for `app/services/semantic_graph.py`, `app/services/docling_parser.py`, `app/services/quality.py`, `app/services/semantic_candidates.py`, `app/services/semantic_generation.py`, `app/services/semantic_governance.py`, and `app/services/runs.py`; the selected-root cases `IC-9A0332D41F79`, `IC-33B4990DC366`, `IC-8AFAD4A415CA`, `IC-865AB8419D55`, `IC-649D7B4E3AB5`, `IC-A92BA42C6D18`, and `IC-81C531769EB3` now reflect that resolved state; accepted routed residuals remain under `IC-6F4E2B5A91C3` for `app/services/semantic_generation_brief.py` at `644` lines and `IC-C8D41A2F77BE` for `app/services/semantic_graph_core.py` at `697` lines plus `app/services/semantic_graph_promotions.py` at `718` lines; `uv run docling-system-improvement-case-validate` is `valid=true`; `uv run docling-system-improvement-case-summary` now reports `case_count=48` with `status_counts.open=32`; `uv run docling-system-hygiene-check` still reports `new hygiene regressions: none`; and the full DB-backed closeout gate passed at `1995 passed`.
- `docs/boring_change_architecture_milestone_plan.md`: refreshed broader coordination brief on 2026-05-18 after the cycle, agent-task residual, DB-model residual, hotspot-routing trap, evaluation, UI, semantic/report, cross-cutting, queue-alignment, documents-service, cross-cutting verification, and governance self-hosting closeouts. The older semantic residual draft remains explicitly superseded, the live `>800` backlog is now cleared, and the broader boring-change brief now treats raw `top_hotspot_paths` as measurement-only while routing the next active code-owning work to a newly reselected under-budget packet.
