...
- `docs/db_models_compatibility_facade_milestone_plan.md`: active bounded milestone brief for the remaining `IC-F2A8110185EB` compatibility-facade / public-import-contract follow-up. Milestone 1 now lands the explicit facade gate and is committed locally as `776fa73`; Milestone 2 remains the ownership-closeout slice for `app/db/models.py`.
...
- Current gate shape: architecture inspection is valid with no violations,
  ...
  `hotspot_count=10`, and `max_hotspot_risk_score=598.8`.
- `app/db/models.py` remains in the architecture-quality routing list, but the
  ...
  model-domain splits, and the remaining 345-line facade now has its own
