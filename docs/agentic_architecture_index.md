...
- `docs/runtime_health_orchestration_milestone_plan.md`: resolved locally through Milestone 4 on 2026-05-14. Milestone 0 refresh / owner-case bootstrap remains committed locally as checkpoint `289f15a`, Milestone 1 gate-first health contract remains committed locally as checkpoint `a84728c`, and Milestones 2 through 4 now close `IC-0F89DBB1CF9F`: `/runtime/status` carries the nested shared `health` report through `app/services/runtime_health.py`, the API/worker/agent-worker publish whole-process heartbeats, `app/runtime_health_cli.py` exposes `docling-system-runtime-health`, `docker-compose.yml` now wires repo-owned healthchecks for all three long-running services with a verified `10s` timeout budget, Compose runtime smoke now passes, and the full DB-backed integration gate is green at `1975 passed`.
- `docs/ci_release_gate_parity_milestone_plan.md`: refreshed locally through Milestone 0 for `IC-2D8D5BF5A8C4` and now the next active follow-on. The draft-time stale routing is replaced with current repo state, the runtime-health dependency is satisfied against the repo-owned `api` / `worker` / `agent-worker` health contract surfaces plus successful Compose smoke, the scoped CI parity gap remains open because only `.github/workflows/architecture-governance.yml` is checked in and there is still no repo-owned `docling-system-release-gate-parity` runner or `.github/workflows/release-gate-parity.yml` workflow, and the Milestone 0 alignment verification is already green (`git diff --check`, improvement-case validate, and improvement-case summary).
