...
- `docs/evidence_residual_owner_family_milestone_plan.md`: active standalone follow-on for the selected residual evidence owners left open after the provenance-export split. Milestone 0 is resolved locally through closeout commit `44bec70`, and Milestone 1 is resolved locally through closeout commit `d9d79ef`: `app/services/evidence_technical_report_exports.py` is reduced to `396` lines, release-binding and audit-bundle or receipt lookup plus provenance-lock assembly now live in `app/services/evidence_technical_report_export_provenance_locks.py` at `426` lines, contract mismatch checks now live in `app/services/evidence_technical_report_export_contracts.py` at `112` lines, the focused export and adjacent-caller unit slice passed at `36 passed`, and the full DB-backed integration gate passed at `2004 passed`; the broader `IC-65AF4A6D8B1E` case remains intentionally open because `app/services/evidence_semantic_trace.py`, `app/services/evidence_claim_feedback.py`, `app/services/evidence_audit_views.py`, `app/services/evidence_manifest_traces.py`, `app/services/evidence_manifests.py`, and `app/services/evidence_claim_support_replay_alerts.py` still measure `837`, `834`, `699`, `980`, `725`, and `646`. Milestone 2, `Technical Report Export Persistence Closeout`, is the next implementation slice.
