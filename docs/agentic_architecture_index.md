...
- `docs/ci_release_gate_parity_milestone_plan.md`: refreshed locally through Milestone 0 for `IC-2D8D5BF5A8C4` and now active through the Milestone 3 local proof checkout after Milestone 2 closeout commit `26dffcd`. The runtime-health dependency is satisfied against the repo-owned `api` / `worker` / `agent-worker` health contract surfaces plus successful Compose smoke, `app/release_gate_cli.py` owns the canonical `docling-system-release-gate-parity` command, focused runner coverage in `tests/unit/test_release_gate_cli.py` now proves the step list, compose-health timeout semantics, and runner-produced report contract at `12 passed`, `.github/workflows/release-gate-parity.yml` uploads the runner-produced `build/release-gate-parity/release_gate_report.json` artifact on every run, the runner still writes bounded failure diagnostics to `build/release-gate-parity/failure/` before Compose teardown, and the remaining scoped gap is the final Milestone 4 closeout alignment.
