...
- `docs/agent_task_schema_aggregation_boundary_milestone_plan.md`: active bounded follow-on after the claim-support, evaluations, evidence provenance-export, semantics, and CLI packets all closed locally. Milestone 0 is now the next active refresh slice for `IC-24F3558D6091` / `app/schemas/agent_tasks.py`, keeping the facade as a compact compatibility surface and moving production imports onto the existing schema owner modules.
