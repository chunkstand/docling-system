...
- `docs/runtime_health_orchestration_milestone_plan.md`: active stacked follow-on after the claim-support, evaluations, evidence provenance-export, semantics, CLI, agent-task schema, oversized-test, hygiene owner-case routing, and architecture-governance cycle packets. Milestone 0 refresh / owner-case bootstrap remains committed locally as checkpoint `289f15a`, and Milestone 1 gate-first health contract is now committed locally as checkpoint `a84728c`: `IC-0F89DBB1CF9F` still anchors the production-orchestration health gap across `app/api/routers/system.py`, `app/services/runtime.py`, `app/workers/poller.py`, `app/workers/agent_poller.py`, and `docker-compose.yml`; the shared owner now lives in `app/services/runtime_health.py` at `256` lines; the public `GET /health` route now returns only bounded `ok` / `error` payloads through the `system_governance` capability seam; and the checked-in architecture workflow now runs `docker compose config --quiet` plus focused runtime-health tests. Authenticated detailed diagnostics, process-heartbeat publication, and Compose healthchecks remain unimplemented, so Milestone 2 hardened API and authenticated runtime-health behavior is now the next active slice.
