...
- `docs/oversized_test_hotspots_boundary_milestone_plan.md`: resolved locally in the 2026-05-14 oversized-test closeout window. The seven selected residual files are now `159`, `328`, `92`, `389`, `117`, `428`, and `93` lines respectively for `tests/db_model_contract.py`, `tests/unit/test_agent_task_context.py`, `tests/unit/test_agent_tasks_api.py`, `tests/unit/test_evaluation_service.py`, `tests/unit/test_search_service.py`, `tests/integration/test_retrieval_learning_ledger.py`, and `tests/integration/test_technical_report_harness_roundtrip.py`; the architecture probe no longer lists any of those residual files in the top 20 hotspots; `IC-5F0E1C8B0D42`, `IC-7A628A4CBCAC`, and `IC-908E7A1D2C44` are now deployed, while `IC-D9A84C20546B`, `IC-3B4C9F2A76E1`, `IC-25C1F7B9E4DA`, and `IC-D49E037D5657` remain reduced/open because focused successor files still exceed the default `600`-line hygiene budget.
- `docs/hygiene_owner_case_routing_boundary_milestone_plan.md`: active bounded follow-on after the claim-support, evaluations, evidence provenance-export, semantics, CLI, agent-task schema, and oversized-test packets. Milestone 0 is now the next required refresh, and it removes the remaining `owner_milestone=residual-weakness-milestone-2` hygiene routing by binding every live ratcheted residual to explicit owner cases and hard-failing future milestone-owned ratchets in the active hygiene contract.
