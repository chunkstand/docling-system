...
- `docs/cli_command_dispatch_boundary_milestone_plan.md`: Milestone 1 is resolved locally after the Milestone 0 rebaseline closeout `381ca15`; the CLI facade-prevention rule now blocks direct session or storage wiring plus parser-body or JSON-render scaffolding in `app/cli.py`, the live hotspot still routes `IC-9812A0B138D9` / `app/cli.py` at `1231` lines / `score 67705`, and Milestone 2 is now the next active CLI command-owner extraction slice. This remains the current active bounded implementation brief.
