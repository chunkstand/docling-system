...
- `docs/evidence_residual_owner_family_milestone_plan.md`: active standalone follow-on for the selected residual evidence owners left open after the provenance-export split. Milestone 0 is resolved locally through closeout commit `44bec70`: `app/services/evidence_technical_report_exports.py` still measures `884` lines, `app/services/evidence_semantic_trace.py` `837`, `app/services/evidence_claim_feedback.py` `834`, and `app/services/evidence_audit_views.py` `699`; the broader `IC-65AF4A6D8B1E` case remains intentionally open because `app/services/evidence_manifest_traces.py`, `app/services/evidence_manifests.py`, and `app/services/evidence_claim_support_replay_alerts.py` still measure `980`, `725`, and `646`; skeletal owner-test seams now exist for claim feedback, semantic trace, and audit views; and the focused Milestone 0 verification slice stayed green. Milestone 1, `Technical Report Derivation And Lock Boundary`, is the next implementation slice.
