...
- `docs/evidence_residual_owner_family_milestone_plan.md`: active standalone follow-on for the selected residual evidence owners left open after the provenance-export split. Milestone 0 is resolved locally through closeout commit `44bec70`, Milestone 1 is resolved locally through closeout commit `d9d79ef`, Milestone 2 is resolved locally through closeout commit `115be15`, and Milestone 3 is resolved locally in the current checkout: `app/services/evidence_claim_feedback.py` is reduced to `498` lines, verdict classification plus retrieval-context, evidence-ref, and desired-row payload shaping now live in `app/services/evidence_claim_feedback_payloads.py` at `376` lines, the focused claim-feedback and adjacent-caller unit slice passed at `36 passed`, the technical-report harness roundtrip slice passed at `1 passed`, and the full DB-backed integration gate passed at `2008 passed`; the broader `IC-65AF4A6D8B1E` case remains intentionally open because `app/services/evidence_semantic_trace.py`, `app/services/evidence_audit_views.py`, `app/services/evidence_manifest_traces.py`, `app/services/evidence_manifests.py`, and `app/services/evidence_claim_support_replay_alerts.py` still measure `837`, `699`, `980`, `725`, and `646`, and because claim-feedback integrity plus ledger persistence still cohabit in the root owner despite the line-budget drop. Milestone 4, `Claim Feedback Integrity And Ledger Closeout`, is the next implementation slice.
