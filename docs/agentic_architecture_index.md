...
- `docs/evidence_residual_owner_family_milestone_plan.md`: active standalone follow-on for the selected residual evidence owners left open after the provenance-export split. Milestone 0 is resolved locally through closeout commit `44bec70`, Milestone 1 is resolved locally through closeout commit `d9d79ef`, and Milestone 2 is resolved locally through closeout commit `115be15`: `app/services/evidence_technical_report_exports.py` is reduced to `45` lines, derivation package shaping plus claim-derivation row payload shaping now live in `app/services/evidence_technical_report_export_payloads.py` at `258` lines, export persistence plus attachment helpers now live in `app/services/evidence_technical_report_export_lifecycle.py` at `138` lines, the focused export and adjacent-caller unit slice passed at `37 passed`, the technical-report harness roundtrip slice passed at `1 passed`, and the full DB-backed integration gate passed at `2005 passed`; the broader `IC-65AF4A6D8B1E` case remains intentionally open because `app/services/evidence_semantic_trace.py`, `app/services/evidence_claim_feedback.py`, `app/services/evidence_audit_views.py`, `app/services/evidence_manifest_traces.py`, `app/services/evidence_manifests.py`, and `app/services/evidence_claim_support_replay_alerts.py` still measure `837`, `834`, `699`, `980`, `725`, and `646`. Milestone 3, `Claim Feedback Payload And Retrieval Context Boundary`, is the next implementation slice.
