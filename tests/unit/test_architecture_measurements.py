...
def test_architecture_measurement_records_routed_hotspot_fields(
    monkeypatch,
    tmp_path: Path,
) -> None:
    history_path = tmp_path / "history.jsonl"
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {
            "hotspot_count": 2,
            "top_hotspot_paths": ["app/db/models.py", "app/services/agent_tasks.py"],
            "top_routed_hotspot_paths": ["app/services/agent_tasks.py"],
            "routing_trap_paths": ["app/db/models.py"],
    ...
    assert record["measurement"]["top_routed_hotspot_paths"] == [
