...
def _load_revision_module():
    path = (
        ...
        / "alembic"
        / "versions"
        / "0065_claim_support_waiver_closures.py"
    )
    spec = importlib.util.spec_from_file_location(
        "revision_0065_claim_support_waiver_closures",
        path,
    )
    if spec is None or spec.loader is None:
        raise AssertionError("Failed to load 0065 migration module")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_revision_0065_adds_replay_fixture_waiver_closure_governance_event(
    monkeypatch,
) -> None:
    ...
    assert "claim_support_replay_alert_fixture_coverage_waiver_closed" in joined
    assert "claim_support_replay_alert_fixture_coverage_waiver_closure" in joined
