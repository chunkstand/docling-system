...
def _load_revision_module():
    path = (
        ...
        / "alembic"
        / "versions"
        / "0063_claim_support_replay_escalations.py"
    )
    spec = importlib.util.spec_from_file_location(
        "revision_0063_claim_support_replay_escalations",
        path,
    )
    if spec is None or spec.loader is None:
        raise AssertionError("Failed to load 0063 migration module")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_revision_0063_adds_replay_escalation_governance_event(monkeypatch) -> None:
    ...
    assert "claim_support_policy_impact_replay_escalated" in joined
    assert "claim_support_policy_impact_replay_escalation" in joined
