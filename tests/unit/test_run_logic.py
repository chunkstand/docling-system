...
def test_process_run_uses_prior_active_run_for_evaluation_baseline(
    ...
    monkeypatch.setattr("app.services.runs.execute_semantic_pass", lambda *args, **kwargs: None)
    monkeypatch.setattr(
...
def test_process_run_keeps_promoted_run_completed_when_semantics_raises(
    monkeypatch, tmp_path: Path
) -> None:
    document_id = uuid4()
    run_id = uuid4()
