...
from tests.integration import retrieval_learning_ledger_support as ls

UTC = ls.UTC
UUID = ls.UUID
AuditBundleExport = ls.AuditBundleExport
AuditBundleValidationReceipt = ls.AuditBundleValidationReceipt
RetrievalHardNegative = ls.RetrievalHardNegative
RetrievalJudgment = ls.RetrievalJudgment
RetrievalJudgmentSet = ls.RetrievalJudgmentSet
RetrievalLearningCandidateEvaluation = ls.RetrievalLearningCandidateEvaluation
RetrievalLearningCandidateEvaluationRequest = ls.RetrievalLearningCandidateEvaluationRequest
RetrievalTrainingRun = ls.RetrievalTrainingRun
SearchFeedback = ls.SearchFeedback
SearchHarnessEvaluation = ls.SearchHarnessEvaluation
SearchHarnessEvaluationResponse = ls.SearchHarnessEvaluationResponse
SearchHarnessRelease = ls.SearchHarnessRelease
SearchHarnessReleaseResponse = ls.SearchHarnessReleaseResponse
SearchReplayQuery = ls.SearchReplayQuery
SearchReplayRun = ls.SearchReplayRun
SearchRequestResultSpan = ls.SearchRequestResultSpan
SemanticGovernanceEvent = ls.SemanticGovernanceEvent
SimpleNamespace = ls.SimpleNamespace
_make_result = ls._make_result
_make_search_request = ls._make_search_request
datetime = ls.datetime
evaluate_retrieval_learning_candidate = ls.evaluate_retrieval_learning_candidate
materialize_retrieval_learning_dataset = ls.materialize_retrieval_learning_dataset
select = ls.select
uuid4 = ls.uuid4
...
def test_materialize_retrieval_learning_dataset_roundtrip(
    ...
    assert all(
        row["source_payload_sha256"] for row in training_audit_payload["retrieval_hard_negatives"]
    )
    assert any(row["evidence_refs"] for row in training_audit_payload["retrieval_hard_negatives"])
    assert any(
    ...
    assert (
        training_receipt["receipt"]["audit_bundle"]["bundle_id"]
    ...
    training_event = next(
        row for row in governance_events if row.event_kind == "retrieval_training_run_materialized"
    )
    ...
    assert candidate_rows[0].learning_package_sha256 == (candidate_response.learning_package_sha256)
    ...
    assert (
        training_event.event_payload_json["retrieval_training_run"]["training_dataset_sha256"]
