...
- `GET /agent-tasks/claim-support-policy-change-impacts`
- `GET /agent-tasks/claim-support-policy-change-impacts/summary`
- `GET /agent-tasks/claim-support-policy-change-impacts/worklist`
- `GET /agent-tasks/claim-support-policy-change-impacts/alerts`
- `POST /agent-tasks/claim-support-policy-change-impacts/alerts/escalations`
- `GET /agent-tasks/claim-support-policy-change-impacts/{change_impact_id}`
- `POST /agent-tasks/claim-support-policy-change-impacts/{change_impact_id}/replay-tasks`
- `POST /agent-tasks/claim-support-policy-change-impacts/{change_impact_id}/replay-status`
...
- technical-report draft and verification tasks expose a frozen claim/evidence audit bundle through `GET /agent-tasks/{task_id}/audit-bundle`; court-grade completeness requires a signed, hash-chained PROV export receipt, so configure `DOCLING_SYSTEM_AUDIT_BUNDLE_SIGNING_KEY` and `DOCLING_SYSTEM_AUDIT_BUNDLE_SIGNING_KEY_ID`
...
The technical-report harness extends that path into an LLM-ready report workflow. `plan_technical_report` turns semantic evidence and graph memory into a section, claim, and retrieval plan; `build_report_evidence_cards` converts source evidence, tables, facts, and approved graph edges into stable evidence cards; `prepare_report_agent_harness` writes both `report_agent_harness.json` and `document_generation_context_pack.json`, with the context pack carrying the report request, context refs, retrieval plan, evidence cards, source evidence package refs, graph context, claim contract, freshness summary, quality contract, audit refs, and stable hash; `evaluate_document_generation_context_pack` records a pre-generation verifier row, operator run, and evaluation artifact for that pack; `draft_technical_report` requires the latest context-pack gate for the harness to pass and match the current context-pack hash before rendering; and `verify_technical_report` verifies claim traceability, claim-support judgments, graph approval, concept coverage, and refreshed wake-up context before downstream review. `GET /agent-tasks/{task_id}/audit-bundle`, evidence manifests, evidence traces, and PROV exports explicitly surface the context-pack gate chain so the generation input can be audited before and after drafting.

`evaluate_claim_support_judge` replays governed hard-case fixture sets against the technical-report claim-support judge and persists the calibration record in `claim_support_evaluations` plus per-case rows. The fixture set is stored in `claim_support_fixture_sets`, the active calibration requirements are stored in `claim_support_calibration_policies`, and every evaluation records the fixture-set ID/SHA plus policy ID/SHA before the gate outcome is persisted. When `policy_version` is omitted, evaluation resolves the one active policy for `policy_name`; pinned versions must still be active. The task writes a `claim_support_judge_evaluation` artifact, records a judge `KnowledgeOperatorRun`, exposes a typed task context with the fixture-set and policy hashes, and treats failed gates as completed, auditable evaluation results rather than worker failures.

The claim-support policy lifecycle is now governed as `draft_claim_support_calibration_policy -> verify_claim_support_calibration_policy -> apply_claim_support_calibration_policy`. Drafting stores a non-active candidate policy and preserves the current active policy hash. Verification replays fixtures against that draft, includes the latest promoted replay-alert fixture set by default, adds mined failed cases from prior claim-support evaluations by default, records replay-alert and mined-failure summaries, records an operator run plus verifier record, and persists the same evaluation evidence as the direct replay task. By default, verification fails when stale replay escalation receipts have not yet been promoted into fixture coverage, so a policy cannot pass while known replay-alert gaps remain outside the evaluated fixture set. If that coverage requirement is disabled, the task input must include a waiver operator, reason, severity, and timezone-aware expiry within 72 hours, and verification writes a durable `claim_support_replay_alert_fixture_coverage_waiver` artifact with lifecycle metadata, optional remediation owner, and a stable waiver hash. Apply is approval-gated, requires a passed verification for the exact unchanged draft policy, recomputes the waiver hash before activation, blocks expired or overlong replay-alert coverage waivers, requires separate waiver-specific activation approval from an operator different from both the task approver and waiver creator when a verification used an active waiver, retires the prior active policy, activates the verified draft, and records approval metadata, waiver activation approval, verifier ID, fixture hash, replay-alert fixture summary, replay-alert coverage waiver, mined-failure manifest, previous/new policy hashes, and activation reasons. Activation now also writes a `claim_support_policy_activation_governance` artifact with the policy diff, verification fixture-set replay and fixture-set diff, replay-alert fixture summary, replay-alert coverage waiver, mined-failure summary, approval record, waiver activation approval, retired-policy references, signed hash-chain receipt when `DOCLING_SYSTEM_AUDIT_BUNDLE_SIGNING_KEY` is configured, PROV JSON-LD, a linked `claim_support_policy_activated` semantic-governance event, and an embedded policy change-impact report. The change-impact report is also persisted in `claim_support_policy_change_impacts`, where it records prior technical-report claim-support judgments, generated draft tasks, verifier tasks, affected IDs, replay recommendations, a reserved row ID, and a verified payload hash so operators can see exactly which court-grade report artifacts predate the newly active calibration policy. Operators can inspect those rows at `GET /agent-tasks/claim-support-policy-change-impacts`, inspect status counts and stale-open rows at `GET /agent-tasks/claim-support-policy-change-impacts/summary`, load the remediation worklist at `GET /agent-tasks/claim-support-policy-change-impacts/worklist`, load a machine-readable stale/blocked alert feed at `GET /agent-tasks/claim-support-policy-change-impacts/alerts`, record idempotent semantic-governance escalation receipts with `POST /agent-tasks/claim-support-policy-change-impacts/alerts/escalations`, mine escalated replay alerts into fixture candidates with `GET /agent-tasks/claim-support-policy-change-impacts/alerts/fixture-candidates`, promote unconverted candidates into content-addressed fixture sets with `POST /agent-tasks/claim-support-policy-change-impacts/alerts/fixture-promotions`, queue managed replay tasks with `POST /agent-tasks/claim-support-policy-change-impacts/{change_impact_id}/replay-tasks`, and refresh closure with `POST /agent-tasks/claim-support-policy-change-impacts/{change_impact_id}/replay-status`; the Agent Workflows UI now exposes the same worklist with stale-row controls, returned/matching row counts, affected audit-bundle links, replay task links, closure receipt links, and queue/refresh actions. Replay queueing prevalidates the full recommendation set, writes child tasks and impact-row status in one transaction, is idempotent after task creation, verifies replay-plan and terminal closure hashes before status mutation, and keeps replay-required impact rows non-closed until the queued draft and verification tasks produce passed technical-report gate evidence plus an immutable replay closure receipt. Worker finalization also refreshes affected impact rows after replay tasks finish, and zero-replay `no_action_required` rows close during activation with the same immutable closure artifact and `claim_support_policy_impact_replay_closed` semantic-governance event used for completed replay rows. Terminal closures must carry a row-level hash, matching payload hash, matching row status, and `replay_closed_at`. Stale or blocked alert escalation writes immutable `claim_support_policy_impact_replay_escalation` artifacts and `claim_support_policy_impact_replay_escalated` semantic-governance events. Fixture promotion writes immutable `claim_support_policy_impact_fixture_promotion` artifacts and `claim_support_policy_impact_fixture_promoted` semantic-governance events that link source impact rows, escalation receipts, candidate fixtures, and the promoted fixture set; those promotion events refresh existing evidence manifests for affected verification tasks. Technical-report audit bundles, evidence manifests, and evidence traces now surface related policy-change replay status, escalation receipts, and fixture-promotion events so stale support-judge policy impacts remain visible until managed replay closes and promoted alert coverage is traceable afterward. Decision signals now raise claim-support replay rows into the operator posture when any impact is open, stale, or blocked, separately flag stale replay escalation receipts that have not yet been promoted into fixture coverage, and classify replay-alert fixture coverage waivers as active, expiring, expired, unmanaged, high-severity, or ready for fixture-promotion remediation. Retired policy identities cannot be reused as active defaults or redrafted; the database enforces one active policy per policy name with a partial unique index.
...
uv run docling-system-agent-task-create evaluate_document_generation_context_pack --input-json '{"target_task_id":"<harness_task_id>","min_traceable_claim_ratio":1.0,"min_context_ref_count":1,"max_blocked_step_count":0,"require_source_evidence_packages":true,"require_fresh_context":false}'
...
uv run docling-system-agent-task-create evaluate_claim_support_judge --input-json '{"evaluation_name":"claim_support_judge_calibration","fixture_set_name":"default_claim_support_v1","fixture_set_version":"v1","policy_name":"claim_support_judge_calibration_policy","min_support_score":0.34,"min_overall_accuracy":1.0,"min_verdict_precision":1.0,"min_verdict_recall":1.0}'
uv run docling-system-agent-task-create draft_claim_support_calibration_policy --input-json '{"policy_name":"claim_support_judge_calibration_policy","policy_version":"v2","rationale":"publish a stricter replay policy","min_hard_case_kind_count":4,"required_hard_case_kinds":["exact_source_support","wrong_evidence","lexical_overlap_wrong_evidence","missing_traceable_evidence"],"required_verdicts":["supported","unsupported","insufficient_evidence"]}'
uv run docling-system-agent-task-create verify_claim_support_calibration_policy --input-json '{"target_task_id":"<policy_draft_task_id>","fixture_set_name":"default_claim_support_v1","fixture_set_version":"v1","include_replay_alert_fixtures":true,"replay_alert_fixture_limit":100,"require_replay_alert_fixture_coverage":true,"include_mined_failures":true,"mined_failure_limit":20}'
uv run docling-system-agent-task-create apply_claim_support_calibration_policy --input-json '{"draft_task_id":"<policy_draft_task_id>","verification_task_id":"<policy_verification_task_id>","reason":"activate the verified claim-support replay policy"}'
...
uv run docling-system-claim-support-replay-alerts --stale-after-hours 24 --format table
uv run docling-system-claim-support-replay-alerts --stale-after-hours 24 --record-escalations --requested-by operator@example.com
uv run docling-system-claim-support-replay-fixtures --stale-after-hours 24 --format table
uv run docling-system-claim-support-replay-fixtures --stale-after-hours 24 --promote --fixture-set-name claim_support_replay_alert_promotions --requested-by operator@example.com
...
bounded promotion command did not cover every available candidate. Claim-support
policy verification now summarizes promoted replay-alert fixture coverage,
unconverted escalation receipts, and the replay-alert fixtures actually included
in the verification set. With the default coverage requirement enabled, stale
unconverted escalation receipts fail the verifier gate; apply records the
coverage summary, waiver lifecycle metadata, and any waiver-specific activation
approval in activation metrics and governance artifacts.
...
Legacy corpus fixtures used to exercise historical parser behavior live under `tests/fixtures/` and are isolated from runtime configuration.
